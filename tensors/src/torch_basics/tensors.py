...
class TestTensors(unittest.TestCase):
    ...
    def testFlip(self):
        ...
        R = A.flip(0)
        self.assertTrue(torch.equal(A, R.flip(0)))
        R = B.flip(0)  # reverses rows, same size
        self.assertTrue(torch.equal(B, R.flip(0)))
        R = B.flip(1)  # reverses columns, same size
        self.assertTrue(torch.equal(B, R.flip(1)))
        R = B.flip(0, 1)  # reverses rows and columns, same size
        self.assertTrue(torch.equal(B, R.flip(0, 1)))
    ...
    def testSplit(self):
        ...
        self.assertTrue(torch.equal(B[0:2], R[0]))  # first two rows
    ...
    def testXStack(self):
        ...
        R = torch.vstack((A, A))
        self.assertEqual([2, m], list(R.size()))

        R = torch.vstack((B, B))
        self.assertEqual([2 * m, n], list(R.size()))

        R = torch.vstack((C, C))
        self.assertEqual([2 * m, n, k], list(R.size()))

        R = torch.hstack((A, A))
        self.assertEqual([2 * m], list(R.size()))

        R = torch.hstack((B, B))
        self.assertEqual([m, 2 * n], list(R.size()))

        R = torch.hstack((C, C))
        self.assertEqual([m, 2 * n, k], list(R.size()))

        R = torch.dstack((A, A))
        self.assertEqual([1, m, 2], list(R.size()))

        R = torch.dstack((B, B))
        self.assertEqual([m, n, 2], list(R.size()))

        R = torch.dstack((C, C))
        self.assertEqual([m, n, 2 * k], list(R.size()))
    ...
    def testStack(self):
        ...
        F = torch.vstack((A, A))
        self.assertEqual([2, m], list(R.size()))
        self.assertTrue(torch.equal(R, F))

        R = torch.stack((A, A), dim=1)
        self.assertEqual([m, 2], list(R.size()))
        self.assertTrue(torch.equal(A, R[:, 0]))
        self.assertTrue(torch.equal(A, R[:, 1]))
        ...
        self.assertEqual([m, n, k, 2], list(R.size()))
    ...
    def testFlatten(self):
        ...
        self.assertEqual(list(R0.size()), [m * n * k])
        R = C.flatten(0, 1)
        self.assertEqual(list(R.size()), [m * n, k])
        R = C.flatten(1, 2)
        self.assertEqual(list(R.size()), [m, n * k])
    ...
    def testNarrow(self):
        ...
        self.assertEqual(list(R0.size()), [3])
        self.assertEqual(list(R1.size()), [2])
        self.assertEqual(list(R2.size()), [2, 4])
        self.assertEqual(list(R3.size()), [1, n, k])
        self.assertEqual(list(R4.size()), [m, 3, k])

    def testGather(self):
        index = torch.tensor([3, 4, 5])
        R = A.gather(0, index)
        self.assertTrue(torch.equal(A[index], R))
        ...
        R = B.gather(0, index0)
        S = B[0].view(1, -1)
        self.assertTrue((torch.equal(R, S)))
